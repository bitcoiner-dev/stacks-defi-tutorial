...
import { appDetails, microstacksPerSTX, exchangeContractName, contractOwnerAddress } from "../lib/constants"
import { ContractCallRegularOptions, openContractCall } from "@stacks/connect"
import { createAssetInfo, makeContractFungiblePostCondition, makeStandardSTXPostCondition, FungibleConditionCode, uintCV, makeContractSTXPostCondition, makeStandardFungiblePostCondition } from "@stacks/transactions"
...
export default function SwapPage() {
  ...
  const stxToTokenSwap = async (stxAmount: number) => {
    ...
    const minimumTokens = ((microstacks / 1000000) * minimumExchangeRate).toFixed(0)
  ...
  const tokenToStxSwap = async (tokenAmount: number) => {
    if (!address || !exchangeInfo) {
      ...
      const minimumExchangeRateMultiple = 1 - (priceSlippage / 100)
      const inverseExchangeRatio = 1 / exchangeRatio;
      const minimumExchangeRate = inverseExchangeRatio * minimumExchangeRateMultiple
      const minimumStx = (tokens * minimumExchangeRate * microstacksPerSTX).toFixed(0);
    ...
    const options: ContractCallRegularOptions = {
      ...
      network,
      appDetails,
      onFinish: ({ txId }) => addTransactionToast(txId, `Swapping ${tokens} MBE...`),
    }
